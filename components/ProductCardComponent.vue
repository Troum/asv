<template>
  ...
  >
    <div :style="`display: grid; grid-template-columns: ${mobile ? '100%' : $display.width(width, 350)}px; grid-template-rows: ${$display.height(height, 350)}px`">
      ...
      <template v-else>
        <div
            :style="`width: 100%; height: 100%; background-image: url('https://dashboard.asvtrade.lt${product.avatar[0].url}'); background-size: contain; background-repeat: no-repeat; background-position: center center`"
        ></div>
      </template>
    </div>
